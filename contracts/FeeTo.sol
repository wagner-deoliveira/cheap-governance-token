...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[ICheapswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[ICheapswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = ICheapswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(ICheapswapV2Pair(pair).transfer(pair, value));
                ICheapswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(ICheapswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface ICheapswapV2Pair {
